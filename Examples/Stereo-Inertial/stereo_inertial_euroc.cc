...
int main(int argc, char **argv)
{
	...
	if (argc < 6) {
		cerr << endl
				// TODO: IMU may come from right or left camera, with timings from right or left. Have option to adjust IMU timing accordingly if frames have to be discarded
				<< "Usage: ./mono_euroc path_to_vocabulary path_to_settings path_to_left_video_file path_to_right_video_file (path_to_imu_file) left_frames_to_discard_may_be_negative) (frame_skip) (trajectory_file_name)"
				<< endl;
		return 1;
	}
	...
	std::string path_to_imu = argv[5];
	int offset = atoi(argv[6]);
	int frame_skip = atoi(argv[7]);
	string path_to_trajectory_output = argv[8];


	// Create SLAM system. It initializes all system threads and gets ready to process frames.
	ORB_SLAM3::System SLAM(path_to_vocab, path_to_settings, ORB_SLAM3::System::IMU_STEREO,
			true);
	...
	for (int i = 0; true; i++) {
		...
		if ((i+1) % (frame_skip + 1) == 0) {
			...
			SLAM.TrackStereo(leftFrame, rightFrame, i * (1 / fps), vImuMeasSlice);
